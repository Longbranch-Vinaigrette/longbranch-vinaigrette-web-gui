...
export default class ComprehensiveStorage {
	...
	createUnit(route, data = undefined, alias = undefined) {
		/**Create unit
		 *
		...
		if (alias === undefined) {
			...
			this.units[route] = new Unit(route, data, alias, this.backendUrl);
		} else {
			// Use the given alias as the unit key.
			this.units[alias] = new Unit(route, data, alias, this.backendUrl);
		}

		return this.units[route].getData();
	}
